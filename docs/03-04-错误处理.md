...
1.  **HTTPException**: 处理 Hono 框架内部异常（如 400 Bad Request），保留状态码并补齐 `requestId`。
2.  **AppError**: 处理业务异常，通过 `mapErrorToResponse` 转换为标准 JSON 响应。
3.  **未知错误**: 捕获未预知的代码异常，统一包装为 `InternalServerError` (500) 进行脱敏。
