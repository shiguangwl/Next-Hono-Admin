...
function printUsage(): void {
  ...
  console.error('   pnpm admin:reset-pwd --password <新密码> --username <新用户名>')
  console.error('')
  console.error('示例:')
  console.error('   pnpm admin:reset-pwd --password MyNewP@ss123')
  console.error('   pnpm admin:reset-pwd --password MyNewP@ss123 --username sysroot')
}
...
async function checkUsernameConflict(
  ...
  const conflict = await db
    ...
    .from(sysAdmin)
    .where(and(eq(sysAdmin.username, newUsername), ne(sysAdmin.id, SUPER_ADMIN_ID)))
    .limit(1)
    .then((rows) => rows[0])
...
async function updateCredentials(
  ...
  await db.update(sysAdmin).set(updateData).where(eq(sysAdmin.id, SUPER_ADMIN_ID))

  // WHY: 密码/用户名变更后吊销所有 Session，防止旧凭证持有者继续访问
  const revokeResult = await db
    .update(sysAdminSession)
    .set({ revokedAt: new Date() })
    .where(and(eq(sysAdminSession.adminId, SUPER_ADMIN_ID), isNull(sysAdminSession.revokedAt)))
...
async function main() {
  ...
  try {
    ...
    if (opts.username && opts.username !== admin.username) {
      console.log(`   用户名已从 "${admin.username}" 修改为 "${opts.username}"`)
    }
