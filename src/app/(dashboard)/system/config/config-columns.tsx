...
export function buildColumns({ onEdit, onDelete }: ColumnActions): ColumnDef<Config>[] {
  return [
    ...
    { key: "configGroup", title: "分组" },
    {
      key: "configKey",
      title: "Key",
      render: (v) => <Code style={{ fontSize: "0.75rem" }}>{v as string}</Code>,
    },
      ...
      render: (_, record) => (
        <Group gap={4}>
          <PermissionGuard permission="system:config:update">
            <ActionIcon variant="subtle" size="sm" onClick={() => onEdit(record)} aria-label="编辑值">
              <Pencil size={14} />
            </ActionIcon>
          </PermissionGuard>
          <PermissionGuard permission="system:config:delete">
            <ActionIcon variant="subtle" color="red" size="sm" disabled={record.isSystem === 1} onClick={() => onDelete(record)} aria-label="删除">
              <Trash2 size={14} />
            </ActionIcon>
          </PermissionGuard>
        </Group>
