...
function SidebarMenuItem({
  ...
  if (collapsed) {
    ...
    if (menu.path && menu.isExternal === 1) {
      btn = (
        <ActionIcon component="a" href={menu.path} target="_blank" variant={variant} size="lg">
          {icon}
        </ActionIcon>
      )
    } else if (menu.path) {
      btn = (
        <ActionIcon component={Link} href={menu.path} variant={variant} size="lg">
          {icon}
        </ActionIcon>
      )
    } else {
...
export function AppSidebar({ collapsed = false, onCollapsedChange }: AppSidebarProps) {
  ...
  return (
    <>
      <AppShell.Section>
        <Group h={59} px={collapsed ? 0 : 'md'} justify={collapsed ? 'center' : 'flex-start'}>
          <Text component={Link} href="/dashboard" fw={700} size="lg" td="none" c="inherit">
            {collapsed ? 'A' : 'Admin'}
          </Text>
        </Group>
        <Divider />
      ...
      <AppShell.Section>
        ...
        <Group justify="center" p="xs">
          <ActionIcon variant="subtle" size="lg" onClick={() => onCollapsedChange?.(!collapsed)}>
