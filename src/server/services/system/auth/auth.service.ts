...
function extractPermissions(adminId: number, menus: Awaited<ReturnType<typeof getGrantedMenus>>) {
  if (adminId === SUPER_ADMIN_ID) return ['*']
  return [...new Set(menus.flatMap((menu) => (menu.permission ? [menu.permission] : [])))]
}
