...
export async function getMenuList(options: MenuQuery = {}): Promise<MenuVo[]> {
  ...
  const menus = await db.select().from(sysMenu).where(whereClause).orderBy(desc(sysMenu.sort))
...
/** 批量更新菜单排序 */
export async function batchUpdateMenuSort(items: BatchSortItem[]): Promise<void> {
  if (items.length === 0) return

  await db.transaction(async (tx) => {
    for (const item of items) {
      await tx.update(sysMenu).set({ sort: item.sort }).where(eq(sysMenu.id, item.id))
    }
  })
